     ...
     { 
         ...
         { 
             ...
             if(received.substring(1).equals("Quit")){ 
                 ...
                 for(int i=0;i<Server_2.ar.size();i++) {
                	 if(this.equals(Server_2.ar.get(i))) {
                		 Server_2.names.remove(i);
                		 Server_2.ar.remove(i);
            	 ...
            	 String x="[";
            	 for(int i=0;i<Server_2.names.size();i++) {
                	 if(Server_2.names.get(i).length()<10) {
                		 x=x+Server_2.names.get(i)+", ";
                	 }
                 }
            	 x=x+"]";
            	 dos.writeUTF(x);
             ...
             {
            	 for (ClientHandler mc : Server_2.ar){
            	 ...
            	 boolean exists=false;
             for (int j=0;j<Server_2.names.size();j++)  
             { 
                 // if the recipient is found, write on its 
                 // output stream 
            	 
                 if (Server_2.names.get(j).equals(Destination) && Server_2.ar.get(j).isloggedin==true)  
                 { 
                	 if(this.name.length()>10) {
                		 StringTokenizer b = new StringTokenizer(this.name, "#"); 
                         String b1 = b.nextToken();
                         Server_2.ar.get(j).dos.writeUTF(b1.substring(1)+" : "+MsgToSend);
                	 }
                	 else
                		 Server_2.ar.get(j).dos.writeUTF(this.name+" : "+MsgToSend);
                     exists=true;
                     break; 
                 } 
             ...
             for (int j=0;j<Server_2.names.size();j++) {
            	 if (Server_2.names.get(j).equals(source.substring(1))) {
            		 Server_2.ar.get(j).dos.writeUTF(source.substring(1)+" : "+MsgToSend);
            	 }
             }
             if(!exists) {
            	 ...
            	 if(received.charAt(0)=='2') {
                     ...
                     { 
                         ...
                         if (Server_2.names.get(j).equals(source.substring(1)) && Server_2.ar.get(j).isloggedin==true)  
                         { 
                        	 if(this.name.length()>10) {
                        		 StringTokenizer b = new StringTokenizer(this.name, "#"); 
                                 String b1 = b.nextToken();
                                 Server_2.ar.get(j).dos.writeUTF("("+b1.substring(1)+" : "+MsgToSend+") was not sent due to not existence of destination");
                        	 }
                        	 else
                        		 Server_2.ar.get(j).dos.writeUTF(this.name+" : "+MsgToSend);
                             exists=true;
